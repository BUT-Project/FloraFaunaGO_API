    ...
    IGenericRepository<TEntity> where TEntity : class
{
    ...
    {
        if (Set.Entry(item).IsKeySet) return Task.FromResult<TEntity?>(null);
    ...
    public virtual Task<TEntity?> Update(string id, TEntity item)
    {
        ...
        if (originalEntity == null) return Task.FromResult<TEntity?>(null);
    ...
    {
        return GetItems(filter, null, index, count, includeProperties);
    }
    ...
    public virtual Pagination<TEntity> GetItems(Func<IQueryable<TEntity>, IQueryable<TEntity>>? orderBy = null,
        ...
        params string[] includeProperties)
    {
        return GetItems(null, orderBy, index, count, includeProperties);
    }

    public virtual Pagination<TEntity> GetItems(int index = 0, int count = 10,
        params string[] includeProperties)
    {
        return GetItems(null, null, index, count, includeProperties);
    }
